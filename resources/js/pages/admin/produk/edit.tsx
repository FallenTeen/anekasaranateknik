...
interface FormData {
  ...
  delete_main_image?: boolean;
}

export default function ProdukEdit({ produk, kategori }: Props) {
  const [previewImage, setPreviewImage] = useState<string | null>(
    produk.gambar ? `/storage/${produk.gambar}` : null
  );
  const [previewImages, setPreviewImages] = useState<{ url: string; name: string; type: 'existing' | 'new' }[]>(
    produk.gambar_deskripsi?.map(img => ({
      url: `/storage/${img.url}`,
      name: img.caption || 'Deskripsi Gambar',
      type: 'existing'
    })) || []
  );
  ...
  const { data, setData, post, processing, errors } = useForm<FormData>({
    ...
    diskon: produk.diskon.toString(),
    kategori: produk.kategori || '',
    display: produk.display,
    status_rekomendasi: produk.status_rekomendasi,
    existing_images: produk.gambar_deskripsi?.map(img => img.url) || []
  });
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setData('gambar', file);
      setData('delete_main_image', false);
  ...
  const removeMainImage = () => {
    setData('gambar', null);
    setData('delete_main_image', true);
  ...
  const removeAdditionalImage = (index: number) => {
    ...
    if (image.type === 'existing') {
      ...
      const urlPart = image.url.replace('/storage/', '');
      const existingIndex = newExisting.indexOf(urlPart);
      ...
      const newIndex = previewImages.slice(0, index).filter(img => img.type === 'new').length;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (data.existing_images) {
      data.existing_images.forEach((url, index) => {
        formData.append(`existing_images[${index}]`, url);
      });
    }

    router.put(route('admin.produk.kelola.update', produk.id), formData as any, {
      forceFormData: true,
    });
  };
  ...
  const breadcrumbs = [
    ...
    { title: 'Edit Produk', href: route('admin.produk.kelola.edit', produk.id) },
  ];

  return (
    <AppSidebarLayout breadcrumbs={breadcrumbs}>
      <Head title={`Edit Produk - ${produk.nama_barang}`} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    />
                    {errors.nama_barang && (
                      <p className="text-sm text-red-500 mt-1">{errors.nama_barang}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {errors.kode_barang && (
                      <p className="text-sm text-red-500 mt-1">{errors.kode_barang}</p>
                    )}
                  </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    <CardTitle className="text-base mb-4">Kategori</CardTitle>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      />
                      {errors.harga_beli && (
                        <p className="text-sm text-red-500 mt-1">{errors.harga_beli}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      />
                      {errors.harga_jual && (
                        <p className="text-sm text-red-500 mt-1">{errors.harga_jual}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    />
                    {errors.diskon && (
                      <p className="text-sm text-red-500 mt-1">{errors.diskon}</p>
                    )}
                    ...
                    {parseFloat(data.harga_jual) > 0 && parseFloat(data.diskon) > 0 && (
                      <div className="mt-2 p-3 bg-gray-50 rounded-lg">
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          <p className="text-sm text-right">
                            {(() => {
                              const margin = ((parseFloat(data.harga_jual) * (1 - parseFloat(data.diskon) / 100) -
                              parseFloat(data.harga_beli)) /
                              parseFloat(data.harga_beli) * 100);
            ...
            <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="mt-2">
                      <label htmlFor="gambar_deskripsi" className="block">
                        <div className="w-full border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center cursor-pointer hover:border-gray-400 py-8">
                          <div className="text-center">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <Label>Tampilkan di Frontend</Label>
                      <p className="text-sm text-gray-600">Produk akan terlihat oleh customer</p>
                    </div>
                    <Switch
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">Tampilkan di section rekomendasi</p>
                    </div>
                    <Switch
              ...
              <div className="space-y-3">
                ...
                <Link href={route('admin.produk.kelola')} className="block">
